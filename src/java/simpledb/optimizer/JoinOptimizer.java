...
public class JoinOptimizer {
    ...
    public static OpIterator instantiateJoin(LogicalJoinNode lj,
        ...
        if (lj.p == Predicate.Op.EQUALS) {
            ...
            try {
                ...
                Class<?> c = Class.forName("simpledb.execution.HashEqualJoin");
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // nested-loops join.
            return (j != null && j.p == Predicate.Op.EQUALS) ? cost1 + cost2 + card1 * 1.2 + card2 :
        ...
        return switch (joinOp) {
            ...
            case EQUALS -> {
                if (t1pkey && t2pkey) {
                    ...
                    yield Math.max(card1, card2);
                } else {
                    yield t1pkey ? card2 : card1;
                }
            }
            case NOT_EQUALS -> {
                yield card1 * card2 - estimateTableJoinCardinality(Predicate.Op.EQUALS, table1Alias, table2Alias, field1PureName,
                        field2PureName, card1, card2, t1pkey, t2pkey, stats, tableAliasToId);
            }
            default -> {
                yield (int) (0.3 * card1 * card2);
            }
        };
    }
...
//        }
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= this.joins.size(); i++) {
            ...
            // 集合大小为 i 时的最优代价
            CostCard currCostCard = new CostCard();
            ...
            for (Set<LogicalJoinNode> set : subsets) {
                ...
                for (LogicalJoinNode logicalJoinNode : set) {
                    CostCard betterCostCard = computeCostAndCardOfSubplan(stats, filterSelectivities,
                            logicalJoinNode, set, minCost, planCache);

                    if (betterCostCard != null) {
                        minCost = betterCostCard.cost;
                        currCostCard = betterCostCard;
                    }
                }

                if (minCost != Double.MAX_VALUE) {
                    planCache.addPlan(set, currCostCard.cost, currCostCard.card, currCostCard.plan);
                }
            }
        ...
        assert ret != null;
        if (explain) {
            printJoins(ret.plan, planCache, stats, filterSelectivities);
        }
        return ret.plan;
    }
