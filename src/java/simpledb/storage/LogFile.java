    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                Long start = this.tidToFirstLogRecord.get(tid.getId());
                ...
                while (true) {
                    try {
                        ...
                        switch (type) {
                            // 这是我们需要做的，但是有个问题，日志记录是顺序记录的，但我们回滚需要逆序回滚
                            // 由于是整页回滚，因此这里我们利用一个集合，第一次回滚，后面就不需要了
                            case UPDATE_RECORD:
                                Page before = readPageData(raf);
                                Page after = readPageData(raf);
                                ...
                                if (recordTid == tid.getId() && !set.contains(pid)) {
                                    set.add(pid);
                                    // 丢弃当前页面，写入回滚页面
                                    // 似乎 pool 没有写入页面的逻辑，因此这里先落盘，然后驱逐页面，让其他事务读取新页
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                //对以提交的事务应用 after image,对未提交的事务应用 before image
                Set<Long> committedTransactionSet = new HashSet<>();
                Set<Long> allTransactionSet = new HashSet<>();
                ...
                while (true) {
                    try {
                        ...
                        switch (type) {
                            case UPDATE_RECORD:
                                allTransactionSet.add(recordTid);
                                Page before = readPageData(raf);
                                Page after = readPageData(raf);
                                beforeImageMap.putIfAbsent(recordTid, new ArrayList<>());
                                afterImageMap.putIfAbsent(recordTid, new ArrayList<>());
                                beforeImageMap.get(recordTid).add(before);
                                afterImageMap.get(recordTid).add(after);
                                break;
                            case CHECKPOINT_RECORD:
                                // 跳过这个东西
                                int numXActions = raf.readInt();
                            ...
                            case COMMIT_RECORD:
                                allTransactionSet.add(recordTid);
                                committedTransactionSet.add(recordTid);
                                break;
                            default:
                                break;
                        }
